...
always@( posedge Clk or negedge Rst_n ) begin
    ...
    else if ( scl_cnt == SCL_CNT_M >> 1 ) begin
        Scl <= 1'b0;
    end
    else if ( scl_cnt == 16'd0 ) begin
        Scl <= 1'b1;
    end
    else begin
        Scl <= Scl;
...
always@( posedge Clk or negedge Rst_n ) begin
    ...
    else if ( scl_cnt == ( SCL_CNT_M >> 2 ) ) begin
        scl_high <= 1'b1;
    end
    else begin
        scl_high <= 1'b0;
...
always@( posedge Clk or negedge Rst_n ) begin
    ...
    else if ( scl_cnt == ( SCL_CNT_M >> 1 ) + ( SCL_CNT_M >> 2 ) ) begin
        scl_low <= 1'b1;
    end
    else begin
        scl_low <= 1'b0;
...
always@( posedge Clk or negedge Rst_n ) begin
    ...
    else if ( ( halfbit_cnt == 8'd16 ) && scl_high && ( Sda == 1'b0 ) ) begin
        ack <= 1'b1;
    end
    else if ( ( halfbit_cnt == 8'd17 ) && scl_low ) begin
        ack <= 1'b0;
    end
    else begin
        ack <= ack;
...
//输出串行数据任务
task send_8bit_data;
    if ( scl_high && ( halfbit_cnt == 8'd16 ) ) begin
        FF <= 1;
    end
...
task receive_8bit_data;
    if ( scl_low && ( halfbit_cnt == 8'd15 ) ) begin
        FF <= 1;
    end
